...
func TestDefaultConfig_Model(t *testing.T) {
	...
	if cfg.Agents.Defaults.Model != "llama3" {
		t.Errorf("Model = %q, want %q", cfg.Agents.Defaults.Model, "llama3")
	}
}
...
func TestSaveConfig_IncludesEmptyLegacyModelField(t *testing.T) {
	...
	if !strings.Contains(string(data), `"model": "llama3"`) {
		t.Fatalf("saved config should include model field with default value, got: %s", string(data))
	}
}
