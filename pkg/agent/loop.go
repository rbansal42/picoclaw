  ...
  Max iterations: %d`, agent.Model, agent.ID, msg.Channel, len(history), agent.MaxIterations), true
	...
	case "/doctor":
		...
		if len(repaired) > len(history) {
			cmdParts = append(cmdParts, fmt.Sprintf("injected %d synthetic tool result(s)", len(repaired)-len(history)))
		}
		if len(repaired) < len(history) {
			cmdParts = append(cmdParts, fmt.Sprintf("dropped %d orphaned tool result(s)", len(history)-len(repaired)))
		}

		return fmt.Sprintf("Session repaired: %s. Messages: %d -> %d.",
			strings.Join(cmdParts, ", "), len(history), len(repaired)), true
